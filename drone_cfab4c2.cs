...
public class DroneAI : Agent
{
    ...
    public override void Initialize()
    {
        ...
        // Ja no busquem un jugador fix aqu├¡, ho farem din├ámicament a l'UpdateStateMachine
    }

    public override void OnEpisodeBegin()
    {
        // Al comen├ºar un nou entrenament, tornem a casa
    ...
    {
        // Nom├®s donem dades ├║tils si estem perseguint
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if (currentState == DroneState.CHASING && jugadorTarget != null)
        {
            ...
            AddReward(-0.01f); // Penalitzaci├│ per temps
        }
        ...
        else if (currentState == DroneState.RETURNING_HOME)
        {
            ...
            {
                Debug.Log("Misi├│n completada. Reiniciando episodio.");
                EndEpisode(); 
            }
        }
        else // IDLE o WAITING
    ...
    void UpdateStateMachine()
    {
        ...
        foreach (GameObject pGo in allPlayers)
        {
            ...
            if (p != null && p.banderaAgafada != null)
            {
                ...
                // Si la bandera que porta ├®s la del MEU equip (teamId), ┬ía per ell!
        ...
        {
            // Si ning├║ t├® la nostra bandera, el dron torna a casa si estava perseguint
    ...
    void MoveTowardsTarget(Vector3 target)
    {
        ...
        rb.linearVelocity = dir * (flySpeed * 1.5f); // Una mica m├®s r├ápid al tornar
    }
    ...
    void DropDino()
    {
        ...
        if (prefabAUsar != null && basePunt != null)
        {
            ...
            // FOR├çAR VISIBILITAT
            nouDino.transform.localScale = new Vector3(4f, 4f, 1f); // Mida est├ándard de les banderes
            ...
            Debug.Log($"<color=green>┬íDINOSAURIO RECUPERADO!</color> ({teamId}) en la base.");
        }
        else
        {
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (currentState == DroneState.CHASING && collision.gameObject.CompareTag("Player"))
        {
            ...
            if (p != null && p.banderaAgafada != null)
            {
                // Verifiquem que la bandera que porta ├®s realment la nostra (per seguretat extra)
                Bandera b = p.banderaAgafada.GetComponent<Bandera>();
                if (b != null && b.equipPropietari == teamId)
                {
                    Debug.Log("┬íCapturat! El dron recupera el dinosaure del seu equip.");
                    AddReward(10.0f);
        ...
        {
            AddReward(-2.0f); // Penalitzaci├│ per xocar
        }
    }
